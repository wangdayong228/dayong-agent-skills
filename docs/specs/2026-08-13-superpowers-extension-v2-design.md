...
**状态：** 已批准（2026-08-13；LLM 非门禁原则于 2026-08-14 再确认）
**日期：** 2026-08-13
...
Plan Assistant 的主要目的是由 agent 承担耗时且难以理解的全量技术检查，并从整个
Plan 中筛出少量必须由用户核实的事项。用户无需为了寻找这些事项而逐行检查全部
Plan 或逐项确认技术 findings。

报告默认只提供分析和 findings，不包含 READY、hash 或其它执行准入状态。报告固定
以“需要用户核实”摘要开头，随后包含 Plan Task 辅助分析、Plan 技术检查、DAG 与
并行建议三个技术章节。
...
- 类别；
- Plan 中的具体 Task、Step 或章节位置；
- 为什么不能由 agent 进行技术判断；
- 需要用户回答的一个明确问题。

没有此类事项时写明“无”。技术 findings、Task 分析细节和并行建议不得混入本节。
正常审阅时，用户只需阅读本节；后三个技术章节供 agent 处理和追溯，用户无需逐项
核实。是否需要暂停或取得用户决定，仍由原版 Superpowers 的既有规则决定，本摘要
不新增批准或执行门禁。
...
- Plan 技术检查只执行第 5.4 节规定的一次修正和一次局部复查；
- 少量代理行为冒烟检查只用于发现明显触发、理解或越界问题，不证明行为稳定性、
  不统计通过率，也不作为交付准入条件；
- LLM 行为不可控，因此不运行以“必须观察到失败”作为继续条件的无-skill LLM
  baseline；RED 证据来自确定性测试，enabled smoke 只作一次性观察；
- 每个预先确定的代理行为场景只采样一次，结果仅为“未发现明显问题”、
  “发现明确越界”或“无法判断”；
- 单次措辞、格式或随机差异不得触发 skill 修改、重复采样或全量重跑；
- 每个明确越界最多允许一次针对性修改，并只复查受影响场景一次；复查后仍越界或
  无法判断时，记录结果与剩余风险并结束本轮 smoke，不再继续测试；该结果不影响
  提交或交付结论；
...
- Plan Assistant 由 agent 承担全量技术检查，并把需要用户核实的事项集中在报告首节；
- 所有循环均有明确退出条件和最大次数；
