...
| `typed-sdk-from-openapi` | 输入任意可信且 pinned 的 OpenAPI 3.x 文档，先调用 `api-client-generator` 生成 raw 层，再产出 2 层 Go SDK（`generated/` + `pkg/client/`，`internal/transport/` 作为内部实现）；中间产物落到 `.sdkgen/`，NO-GO fail-fast 仅输出报告。 |
